...
class DFRobotVL53L0X:
  ...
  def setDeviceAddress(self, newAddr):
    newAddr &= 0x7F
    x = (VL53L0X_REG_I2C_SLAVE_DEVICE_ADDRESS,newAddr)
    self.writeData(x)
  ...
  def start(self):
    ...
    if DeviceMode == VL53L0X_DEVICEMODE_SINGLE_RANGING:
      x = (VL53L0X_REG_SYSRANGE_START,1)
      self.writeData(x)
      Byte = StartStopByte
      LoopNb = 0
      while True:
      ...
      x = (VL53L0X_REG_SYSRANGE_START,VL53L0X_REG_SYSRANGE_MODE_BACKTOBACK)
      self.writeData(x)
    else:
      print("---Selected mode not supported---")
 
  def stop(self):
    x = (VL53L0X_REG_SYSRANGE_START,VL53L0X_REG_SYSRANGE_MODE_SINGLESHOT)
    self.writeData(x)
    for x in [(0xff,1),(0,0),(0x91,0),(0,1),(0xff,0)]:
      self.writeData(x)
	  
  def setRangeFractionEnable(self, NewState):
    x = (VL53L0X_REG_SYSTEM_RANGE_CONFIG,NewState)
    self.writeData(x)
  ...
  def DataInit(self):
    if ESD_2V8 == 1:
      ...
      data = (data & 0xFE) | 0x01
      x = (VL53L0X_REG_VHV_CONFIG_PAD_SCL_SDA__EXTSUP_HV,data)
      self.writeData(x)
